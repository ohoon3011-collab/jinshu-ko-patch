...
function Patch-LuaTextAssembly([string]$Root, [string]$FileName, $Pairs, [string]$MarkerBase64) {
    ...
    foreach ($pair in $Pairs) {
        ...
        if ($source.Contains($oldText)) {
            ...
            $changed++
        } elseif ($oldText.Contains("`n")) {
            # Decompiled reference files and live game scripts can use
            # different newline styles.  Accept CRLF without weakening the
            # exact anchor match, so multi-line structural patches stay live.
            $oldCrLf = $oldText.Replace("`r`n", "`n").Replace("`n", "`r`n")
            if ($source.Contains($oldCrLf)) {
                $newCrLf = $newText.Replace("`r`n", "`n").Replace("`n", "`r`n")
                $source = $source.Replace($oldCrLf, $newCrLf)
                $changed++
            }
        }
    }
...
function Patch-WarDisplay([string]$Root) {
    ...
    $warPairs = @(
        ...
        # The initiative bar draws names vertically after splitting them into
        # individual glyphs, so translate the complete character name first.
        ,@('ZnVuY3Rpb24gZHJhd25hbWUoeCwgeSwgaWQsIHNpemUpCiAgICBsb2NhbCBuYW1lID0gSlkuUGVyc29uW2lkXVsi5aeT5ZCNIl0=', 'ZnVuY3Rpb24gZHJhd25hbWUoeCwgeSwgaWQsIHNpemUpCiAgICBsb2NhbCBuYW1lID0gS09UUihKWS5QZXJzb25baWRdWyLlp5PlkI0iXSBvciAiIik=')
...
function Patch-WarStatistics([string]$Root) {
    # The statistics title and page-one headers are baked into data/xt/588.png.
    # Cover just those text bands and redraw Korean labels; later pages use
    # direct Korean strings so dynamic headings and footer text stay complete.
    $statPairs = @(
        ,@('ICAgICAgICBMb2FkUE5HX0FscGhhKDkxLCA1ODggKiAyLCAtMSwgLTEsIDMsIDI1NSwgMTAwKQogICAgICAgIC0tIOaVjOaIkeaYoOWwhCArIOaUtumbhuaciee7n+iuoeiusOW9leeahOWPguaImOiAhQ==', '')
        ,@('ICAgICAgICAgICAgc2lkZUluZm8gPSBzdHJpbmcuZm9ybWF0KCIgIOaIkeaWuei+k+WHujolZCDmlYzmlrnovpPlh7o6JWQiLCB0b3RbMV1bMV0sIHRvdFsyXVsxXSk=', 'ICAgICAgICAgICAgc2lkZUluZm8gPSBzdHJpbmcuZm9ybWF0KCIgIOyVhOq1sCDtlLztlbQ6JWQgIOyggeq1sCDtlLztlbQ6JWQiLCB0b3RbMV1bMV0sIHRvdFsyXVsxXSk=')
        ,@('', '')
...
function Patch-ItemMenuLayout([string]$Root) {
    # The item screen measures the short Chinese source before DrawString
    # translates it. Measure the final Korean text instead so requirements
    # wrap before they collide, and replace the long footer with two rows.
    $itemPairs = @(
        ,@('ICAgICAgICAgICAgICAgICAgICAgICAgICAgIGlmIG15cyB+PSBuaWwgdGhlbgogICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgIGxvY2FsIG15bGVuID0gbXlmb250ICogc3RyaW5nbGVuKG15cykgLyAyICsgMTI=', 'ICAgICAgICAgICAgICAgICAgICAgICAgICAgIGlmIG15cyB+PSBuaWwgdGhlbgogICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgIG15cyA9IEtPVFIobXlzKQogICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgIGxvY2FsIG15bGVuID0gbXlmb250ICogc3RyaW5nbGVuKG15cykgLyAyICsgMTI=')
        ,@('', '')
    )
    Patch-LuaTextAssembly $Root 'jymain.lua' $itemPairs 'ICAgICAgICAgICAgICAgICAgICAgICAgICAgIG15cyA9IEtPVFIobXlzKQ=='
}
...
function Patch-DescriptionSources([string]$Root) {
    ...
    $kungfuPairs = @(
        # Build base move effects directly in natural Korean. Translating the
        # completed color-coded sentence token by token produced fragments
        # such as "대대상입힘중독" with missing particles and spaces.
        ,@('', '')
        ,@('', '')
        ,@('ICAgICAgICAgICAgICAgICAgICBzdHJpbmcuZm9ybWF0KCLpgKDmiJDln7rnoYDkvKTlrrMlZOeCueOAgiVzIiwgZmluYWxBdGssIG1vdmVEZXNjKSw=', 'ICAgICAgICAgICAgICAgICAgICBzdHJpbmcuZm9ybWF0KCLquLDrs7gg7ZS87ZW066W8ICVk7KCQIOyehe2emeuLiOuLpC4gJXMiLCBmaW5hbEF0aywgbW92ZURlc2MpLA==')
        # Common move-effect descriptions (shared by every martial art).
        ,@('', '')
        ,@('ICAgICAgICBbMTVdID0geyBuYW1lID0gIuWbuuS8pCIsIHR5cGUgPSAi5Lyk5a6z57G7IiwgdzEgPSAxNTAsIHcyID0gMTU1LCBkZXNjID0gZnVuY3Rpb24oKSByZXR1cm4gc3RyaW5nLmZvcm1hdCgi6ZmE5Yqg6aKd5aSW5Lyk5a6z77ysJWTvvLfngrnjgIIiLCBsZXZlbCAqIDUwKSBlbmR9LA==', 'ICAgICAgICBbMTVdID0geyBuYW1lID0gIuWbuuS8pCIsIHR5cGUgPSAi5Lyk5a6z57G7IiwgdzEgPSAxNTAsIHcyID0gMTU1LCBkZXNjID0gZnVuY3Rpb24oKSByZXR1cm4gc3RyaW5nLmZvcm1hdCgi7LaU6rCA66GcIO+8rCVk77y37J2YIOqzoOyglSDtlLztlbTrpbwg7KSN64uI64ukLiAiLCBsZXZlbCAqIDUwKSBlbmR9LA==')
        ...
        # Fully localized named internal-skill effects that otherwise retain
        # long Chinese clauses after only their titles are translated.
        ,@('ICAgICAgICAgICAgbmFtZSA9ICLkuZ3lub3kv67nvZciLA==', 'ICAgICAgICAgICAgbmFtZSA9ICLqtazsnKDsiJjrnbwiLA==')
        ,@('', '')
        ,@('ICAgICAgICAgICAgbmFtZSA9ICLpmLTnhZ7ngrzni7EiLA==', 'ICAgICAgICAgICAgbmFtZSA9ICLsnYzsgrTsl7DsmKUiLA==')
        ,@('', '')
...
function Invoke-KoreanPatch([string]$Root) {
    ...
    Patch-ItemMenuLayout $Root
    Patch-WarDisplay $Root
    Patch-WarStatistics $Root
